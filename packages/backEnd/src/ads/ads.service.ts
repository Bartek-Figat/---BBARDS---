...
const uploadedFilesToSpaces = async (requsetFiles: Express.Multer.File[]) => {
    ...
    return Promise.all(spacesFiles).then((values) => values.map(({Location}) => Location))
}
...
export class AdsService {
    ...
    async addAd(
        ...
        try {
            ...
            const response = await db
                .collection(Index.Add).insertOne({
                    author: token,
                    productTitle,
    ...
    async filterCategories(categories: Categories) {
        ...
        try {
            ...
            const {
                ...
                await this.advancedFiltration(props);
            return HttpResponse.sucess(
                {
    ...
    async advancedFiltration({
                                 ...
                                 nPerPage,
                                 filterQuery
                             }: { pageNumber: number, nPerPage: number, filterQuery: { [k: string]: any; } }): Promise<any> {
